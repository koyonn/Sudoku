...
public class UserInterfaceImplementation implements UserInterfaceContract.View, EventHandler<KeyEvent> {
	...
	/**
	 * Вспомогательный метод, служащий для инициализации компонентов
	...
	private void initializeUserInterface() {
		...
		drawGridLines(root);
		stage.getIcons()
		     .add(new Image(Objects.requireNonNull(SudokuApplication.class.getResourceAsStream("Icon.png"))));
	...
	private void addButtons(Group root) {
		...
		buttonExit.setStyle("-fx-padding: 17.5; " + "-fx-background-color: black; "
		        + "-fx-text-fill: white; "
		        + "-fx-font-size: 16; "
		...
		buttonRestart.setStyle("-fx-padding: 17.5; " + "-fx-background-color: black; "
		        + "-fx-text-fill: white; "
		        + "-fx-font-size: 16; "
		        + "-fx-border-color: white; "
		        + "-fx-border-radius: 16; "
		...
		root.getChildren()
		    .addAll(buttonExit, buttonRestart);
	}
	...
	private void drawBackground(Group root) {
		Image image = new Image("koyonn/sudoku/background.jpg", 732, 732, false, true);
	...
	private void drawTitle(Group root) {
		...
		title.setStyle("-fx-font: 38px System; " + "-fx-stroke: white; "
		        + "-fx-stroke-width: 1; "
		        + "-fx-font-smoothing-type: lcd;");
		root.getChildren()
		    .add(title);
	...
	private void drawGridLines(Group root) {
		...
		while (index < 8) {
			...
			// Для отрисовки линий поля будет использоваться класс Rectangle
			// (JavaFX)
			// Рисуем вертикальную линию и сдвигаем ее на 64 пикселя
			Rectangle verticalLine = getLine(xAndY + DELTA * index, BOARD_PADDING, BOARD_X_AND_Y, thickness);
			// Рисуем горизонтальную линию и сдвигаем ее на 64 пикселя
			Rectangle horizontalLine = getLine(BOARD_PADDING, xAndY + DELTA * index, thickness, BOARD_X_AND_Y);
			root.getChildren()
			    .addAll(verticalLine, horizontalLine);
			index++;
		}
	}
	...
	/**
	 * Метод для обновления всего поля судоку (например, когда игрок закончил
	 * игру)
	 *
	 * @param game игра
	 */
		...
		for (int xIndex = 0; xIndex < 9; xIndex++) {
			for (int yIndex = 0; yIndex < 9; yIndex++) {
				...
				if (game.getGameState() == GameState.NEW) {
					// Если ячейка пуста, то делаем ее активной, иначе блокируем
					// и делаем шрифт более прозрачным
					if (value.equals("")) {
						tile.setStyle("-fx-opacity: 1; " + "-fx-text-inner-color: white;");
						tile.setDisable(false);
					} else {
						tile.setStyle("-fx-opacity: 0.45; " + "-fx-text-inner-color: white;");
						tile.setDisable(true);
					}
				} else if (game.getGameState() == GameState.ACTIVE) {
					// При повторном запуске, первоначально заполненные ячейки
					// делаем неактивными
	...
	/**
	 * Метод будет вызван, когда logic center получит извещение, что игра
	 * окончена
	 *
	...
	/**
	 * Когда пользователь вводит число в текстовой поле, то это событие появится
	 * тут
	 *
