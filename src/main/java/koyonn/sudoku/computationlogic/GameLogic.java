...
public class GameLogic {
	...
	public static SudokuGame getNewGame() {
		...
		return new SudokuGame(GameState.NEW, array, array);
	}
	 ...
	 */
	public static boolean sudokuValidation(int number, int rowIndex, int rowOffset, int columnIndex, int columnOffset,
	                                       int[][] sudoku) {
		// Проверка блока 3х3
		logger.trace("number={}, rowIndex={}, rowOffset={}, columnIndex={},columnOffset={}", number, rowIndex,
		        rowOffset, columnIndex, columnOffset);
		if (blockCheck(number, rowOffset, columnOffset, sudoku)) {
			logger.debug("blockCheck - есть совпадение");
			return true;
		}
		// Проверка ряда
		...
		if (columnCheck(number, columnIndex, sudoku)) {
			logger.debug("columnCheck - есть совпадение");
			return true;
		}
		logger.debug("sudokuValidation - нет совпадений");
		// Если заполнено правильно и нет совпадений
		return false;
	}

	/**
	 * Метод для проверки совпадений сгенерированного числа с уже записанными числами в блоке 3х3.
	 *
	 * @param number       сгенерированное число
	 * @param rowOffset    отступ по ряду
	...
	/**
	 * Метод для проверки совпадений сгенерированного числа с уже записанными числами в указанном ряду.
	 *
	 * @param number   сгенерированное число
	 * @param rowIndex индекс ряда
	 * @return true - если есть совпадение
	...
	/**
	 * Метод для проверки совпадений сгенерированного числа с уже записанными числами в указанном столбце.
	 *
	 * @param number      сгенерированное число
	...
	/**
	 * Метод, предназначенный для обнуления определенных ячеек. При заполнении массива случается, что в указанную ячейку
	 * невозможно добавить число - есть совпадения как по вертикали так, например, и в блоке. Тогда, в зависимости от
	 * индекса текущей ячейки, ближайшим ячейкам присваивается нулевое значение, и затем заполнение массива
	 * продолжается.
	 *
	 * @param rowIndex    индекс ряда
	 * @param columnIndex индекс столбца
