...
public class SudokuSolver {
	...
	public static boolean puzzleIsSolvable(int[][] puzzle) {
		...
		// Если пустых не найдено, значит судоку заполнено, а следовательно и
		// решаемо
		if (isEmpty) {
			return true;
		}
		...
		for (int input = 1; input <= GRID_BOUNDARY; input++) {
			// Выполнение проверки на совпадение числа в квадрате, строке и
			// столбце
			if (!sudokuValidation(input, row, (row / 3), column, (column / 3), puzzle)) {
				// Если совпадений нет, то число записывается в массив
				puzzle[row][column] = input;
				// Рекурсивно вызываем метод и подставляем в него массив, в
				// который только что было внесено число
				if (puzzleIsSolvable(puzzle)) {
					// Если в какой-то момент выполнения метода случится так,
					// что ни одно число в цикле
					// не удовлетворяет требованиям, то произойдет выход из
					// цикла, значение так и останется 0, и
					// будет возвращен false. Следовательно, будет возврат в
					// прошлый шаг рекурсии, но уже будет
					// выполнено else - обратно присвоено 0 значение и продолжен
					// цикл input, начиная с того значений,
