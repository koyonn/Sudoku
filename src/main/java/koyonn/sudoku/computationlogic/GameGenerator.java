...
public class GameGenerator {
	...
	private static int[][] creatingSudoku() {
		...
		while (nullCheck(sudokuArray)) {
			logger.trace("{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}", Arrays.toString(sudokuArray[0]),
			        Arrays.toString(sudokuArray[1]), Arrays.toString(sudokuArray[2]), Arrays.toString(sudokuArray[3]),
			        Arrays.toString(sudokuArray[4]), Arrays.toString(sudokuArray[5]), Arrays.toString(sudokuArray[6]),
			        Arrays.toString(sudokuArray[7]), Arrays.toString(sudokuArray[8]));
			...
			while (checkNumber) {
				...
				if (!sudokuValidation(number, rowIndex, rowOffset, columnIndex, columnOffset, sudokuArray)) {
					sudokuArray[rowIndex][columnIndex] = number;
					checkNumber = false;
				}
				// Если количество попыток записи достигнет указанного значения,
				// то
	...
	private static int getRandomNumber(int beginIndex, int endIndex) {
		return RANDOM.nextInt(beginIndex, endIndex);
	}
}
